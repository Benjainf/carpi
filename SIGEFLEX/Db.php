...
function Conexion ($Db){ //Crea la funcion
	$Conect=mysqli_connect("localhost","root","","base_de_datos") //Realiza una consulta en donde se almacena los archivos
...
function Ejecutar_Consulta($Db,$Con){ //recibe la consulta y ejecuta la consulta
	$Result=mysqli_query($Db,$Con); //ejecuta la consulta
	return $Result; //devuelve el valor al archivo metadatos
}
...
// Continuando en ResultadoBusqueda.php  
...
// Paso 4: Mostrar los resultados en una tabla HTML  
if ($result->num_rows > 0) {  
    echo "<table>";  
    echo "<tr><th>Nombre</th><th>Apellido</th><th>Teléfono</th><th>Edad</th></tr>"; // Añade más encabezados según tu tabla  
    while ($row = $result->fetch_assoc()) {  
        echo "<tr><td>{$row['nombre']}</td><td>{$row['apellido']}</td><td>{$row['DNI']}</td><td>{$row['edad']}</td></tr>";  
    }  
    echo "</table>";  
} else {  
